...
* `ex_dataclass` 是一款继基于 `dataclass` 开发的 `python` 库并且对数据模型的处理更加友好。它通过  **Python3的类型注解** 特性能轻易的处理 `dict` 与自定义 `class` 间的转换，从而明确复杂数据模型的类型，**降低**维护代码的压力、**降低**操作复杂数据难度及**提高**代码逻辑清晰度。
